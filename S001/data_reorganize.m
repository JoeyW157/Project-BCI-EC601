...
T0 = 0;
% defult, for test R04,08,12 we have T1 = C1, T2 = C2
T1 = 1;
T2 = 2;
if contains(file_path,"R06") || contains(file_path,"R10") || contains(file_path,"R14")
    % otherwise for test R06,10,14 we have T1 = C3, T2 = C4 
    T1 = 3;
    T2 = 4;
end
...
for i=1:size(labels,1)
    % transfer the label into numeric label class
    if strcmp(labels(i,1),"T1")
        label = T1;
    elseif strcmp(labels(i,1),"T2")
        label = T2;
    else
        label = T0;
    end
